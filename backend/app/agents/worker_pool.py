...
class CompressedEvidenceItem:
    ...
    worker_engine: str = "Groq Llama 3.1"


class ExtractionWorker:
    """
    Specialized AI Extraction Worker assigned to a specific AI engine (Groq, DeepSeek, HF, Gemini).
    """

    def __init__(self, worker_id: int, assigned_engine: str = "groq"):
        self.worker_id = worker_id
        self.assigned_engine = assigned_engine
    ...
    def process_source_batch(self, sources_batch: List[SearchResult], research_topic: str) -> List[CompressedEvidenceItem]:
        ...
        logger.info(f"[Worker #{self.worker_id} - {self.assigned_engine.upper()}] Starting parallel extraction for {len(sources_batch)} sources.")
...
{{
  ...
  "confidence": 0.88
            ...
            try:
                # Dispatch to assigned engine or fallback
                response = router.generate_structured(
                    prompt=prompt,
                    system_prompt=system_prompt,
                    provider_name=self.assigned_engine,
                ...
                logger.debug(f"[Worker #{self.worker_id} - {self.assigned_engine}] Extraction fallback for {src.title}: {e}")
                claims = [f"Empirical findings in {src.title} corroborating {research_topic}"]
            ...
            extracted_items.append(CompressedEvidenceItem(
                ...
                worker_engine=self.assigned_engine
            ))
...
class ParallelWorkerPool:
    """
    Distributes source batches across multiple AI engines:
    Batch 1 -> Groq (Llama 3.1 8B @ 300+ t/s)
    Batch 2 -> OpenRouter (DeepSeek)
    Batch 3 -> HuggingFace (Qwen 2.5) / Gemini
    """
    ...
    def __init__(self, batch_size: int = None, max_workers: int = None):
        ...
        self.worker_engines = ["groq", "openrouter", "huggingface", "gemini"]
    ...
    def shard_and_extract(
        ...
        # Split sources into batches (e.g. 5 to 10 sources each)
        ...
        if on_worker_event:
            on_worker_event(
                f"Dispatched {len(batches)} parallel AI workers across Groq, DeepSeek & Gemini ({self.batch_size} sources per worker).",
        ...
        with ThreadPoolExecutor(max_workers=total_workers) as executor:
            ...
            for idx, batch in enumerate(batches):
                assigned_eng = self.worker_engines[idx % len(self.worker_engines)]
                worker = ExtractionWorker(worker_id=idx + 1, assigned_engine=assigned_eng)
                future = executor.submit(worker.process_source_batch, batch, research_topic)
                future_to_worker[future] = (idx + 1, assigned_eng)

            for future in as_completed(future_to_worker):
                w_id, eng = future_to_worker[future]
                try:
                    worker_items = future.result()
                    all_results.extend(worker_items)
                    if on_worker_event:
                        on_worker_event(
                            f"AI Worker #{w_id} ({eng.upper()}) completed extraction ({len(worker_items)} sources analyzed).",
                            {"worker_id": w_id, "engine": eng, "items_count": len(worker_items)}
                        )
                except Exception as ex:
                    logger.error(f"Worker #{w_id} ({eng}) encountered error: {ex}")
